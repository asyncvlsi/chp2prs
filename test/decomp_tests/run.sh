...
echo
echo "************************************************************************"
echo "*                  Testing concurrent decomposition                    *"
echo "************************************************************************"
echo
...
do
	orig=${count}
        ...
        ok=1
        for opt in "-F decomp -X -P 50000.0" "-F decomp -O -X -P 50000.0"
	do
        if [ $ok -eq 1 ]
        then
	$ACTTOOL $opt -E abc -e runs/${orig}_expr.act -p dc${orig} $i > runs/${orig}decomp.act  2>runs/$i.t.stderr
        if test -s runs/$i.t.stderr 
        then
                echo 
                myecho "** FAILED DECOMPOSITION, $opt TEST $i: stderr"
                fail=`expr $fail + 1`
                ok=0
	...
	then
		...
		then
			if [ $code -eq 2 ]
			then
				myecho "** FAILED DECOMPOSITION, TEST $i"
			else
				myecho "** FAILED DECOMPOSITION SIMULATION, $opt TEST $i"
			fi
			fail=`expr $fail + 1`
			ok=0
		fi	
