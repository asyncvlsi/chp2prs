...
*/
#if 0
bool var_appears_in_expr_old (Expr *e, ValueIdx *vx, Scope *s, var_info *v)
{
  act_connection *uid;
  ActId *i;
  ValueIdx *vix;
...
bool var_appears_in_expr (Expr *e, ActId *id, Scope *s, var_info *v)
{
  act_connection *uid;
  ActId *i;
  bool a1, a2, a3;
  ...
  case E_EQ:
  case E_NE:
    a1 = var_appears_in_expr (e->u.e.l, id, s, v);
    a2 = var_appears_in_expr (e->u.e.r, id, s, v);
    return a1 | a2;
    break;
  ...
  case E_QUERY:
    a1 = var_appears_in_expr (e->u.e.l, id, s, v);
    a2 = var_appears_in_expr (e->u.e.r->u.e.l, id, s, v);
    a3 = var_appears_in_expr (e->u.e.r->u.e.r, id, s, v);
    return a1 | a2 | a3;
    break;
  ...
  case E_FUNCTION:
    ...
    e = e->u.fn.r;
    while (e) {
      var_appears_in_expr (e->u.e.l, id, s, v);
      e = e->u.e.r;
    }
    break;
...
*/
#if 0
var_info *construct_var_info_old (act_chp_lang_t *c, ValueIdx *vidx, Process *p, var_info *v, 
...
var_info *construct_var_info (act_chp_lang_t *c, ActId *id, Process *p, var_info *v, 
                              int root, int bitwidth)
{
  // ActId *id;
  ValueIdx *vx;
  InstType *it;
  ...
  case ACT_CHP_ASSIGN:
    ...
    // x = v->nread;
    // _clear_var_flags (v);
    if ( var_appears_in_expr (c->u.assign.e, id, s, v) )
      v->nread++;
    // if (x != v->nread) {
    //   c->type = ACT_CHP_ASSIGNSELF;
    // }
  ...
  case ACT_CHP_SEND:
    ...
    // v->nwrite++;
    if (c->u.comm.e) {
      if ( var_appears_in_expr (c->u.comm.e, id, s, v) )
        v->nread++;
    }
    break;
  ...
  case ACT_CHP_SEMI:
    ...
    for (listitem_t *li = list_first (c->u.semi_comma.cmd); li; li = list_next (li)) {
      v = construct_var_info ((act_chp_lang_t *) list_value (li), id, p, v, 0, bitwidth);
    }
    break;
  ...
  case ACT_CHP_DOLOOP:
    {
      ...
      {
        if (gc->g) {
          if ( var_appears_in_expr (gc->g, id, s, v) )
            v->nread++;
        }
        gc = gc->next;
      }
      ...
      while (gc) {
      // _clear_var_flags ();
      v = construct_var_info (gc->s, id, p, v, 0, bitwidth);
      gc = gc->next;
      }
    }
    break;
...
*/
Hashtable *construct_var_info_hashtable (act_chp_lang_t *c, Process *p, ActBooleanizePass *bp)
{
  ...
  act_boolean_netlist_t *bnl = bp->getBNL(p);
  Assert (bnl, "hmm BNL");
  ...
  while ((ib = ihash_iter_next (pht, &it))) {
    ...
    if (bv->usedchp) {
      ...
      if (TypeFactory::isDataType (vx->t)) 
      {
        // v = construct_var_info (c, vx, p, v, 1, NULL);
        v = construct_var_info (c, id, p, v, 1, bv->width);
...
*/
Hashtable *print_var_info_hashtable (FILE *fp, Hashtable *var_infos)
{
  fprintf (fp, "\nvar_info hashtable: \n");
  hash_bucket_t *b;
  var_info *v;
